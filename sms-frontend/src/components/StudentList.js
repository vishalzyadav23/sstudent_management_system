...
function StudentList() {
  ...
  const deleteStudent = (id) => {
    ...
    if (window.confirm("Are you sure you want to delete this student?")) {
      axios
        .delete(
          `[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/students/${id}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        )
        ...
        .catch((error) => {
          ...
          if (error.response && error.response.status === 403) {
            alert(
              "403 Forbidden: Your Admin session expired! Please click the red 'Logout' button and log back in as Admin.",
            );
          } else if (error.response && error.response.status === 500) {
            alert(
              "500 Error: Cannot delete this student because they have a linked User Account. We need to update the database rules!",
            );
          } else {
  ...
  const processedStudents = useMemo(() => {
    let result = students.filter((student) => {
      ...
      const matchesDept =
        filterDept === "" ||
        (student.department &&
          student.department.toLowerCase() === filterDept.toLowerCase());
  ...
  const itStudents = students.filter(
    (s) =>
      ...
      (s.department.toLowerCase() === "it" ||
        s.department.toLowerCase() === "cs" ||
        s.department.toLowerCase() === "computer science"),
  ).length;
    ...
    <div>
      ...
      >
        ...
        <Link to="/add">
          ...
          >
            <span style={{ fontSize: "16px", marginRight: "8px" }}>+</span> Add
            New Student
          </button>
        </Link>
      </div>
        ...
        <tbody>
          {currentStudents.length > 0 ? (
            currentStudents.map((student) => {
              ...
              const deptUpper = student.department
              ...
              if (
                deptUpper === "IT" ||
                deptUpper === "CS" ||
                deptUpper === "COMPUTER SCIENCE"
              )
                badgeClass = "dept-it";
              else if (deptUpper === "MECHANICAL")
                badgeClass = "dept-mechanical";
              else if (deptUpper === "EC" || deptUpper === "ELECTRONICS")
                badgeClass = "dept-ec";
            ...
            <tr>
              ...
              >
                ...
                <h3 style={{ color: "#334155", margin: "0 0 5px 0" }}>
                  No students found
                </h3>
                <p style={{ color: "#64748b", margin: 0 }}>
