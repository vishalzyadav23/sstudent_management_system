...
function AdminDashboard() {
  ...
  const fetchCourses = () =>
    axios
      .get(
        "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/courses",
  ...
  const fetchFaculty = () =>
    axios
      .get(
        "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/faculty",
        { headers: getAuthHeader() },
      )
  ...
  const fetchStudents = () =>
    axios
      .get(
        "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/students",
        { headers: getAuthHeader() },
      )
      .then((res) => {
        const studentArray = Array.isArray(res.data)
          ? res.data
          : res.data.data || [];
  ...
  const deleteSubject = (id) => {
    if (window.confirm("Are you sure you want to delete this subject?")) {
      axios
        .delete(
          `[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/courses/${id}`,
          { headers: getAuthHeader() },
        )
        .then(() => {
  ...
  const deleteFaculty = (id) => {
    if (window.confirm("Remove this faculty member from the system?")) {
      axios
        .delete(
          `[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/faculty/${id}`,
          { headers: getAuthHeader() },
        )
        .then(() => {
  ...
  const handleEnrollment = (e) => {
    ...
    axios
      .post(
        "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/enroll",
        {
  ...
  const handleBulkPublish = async (e) => {
    ...
    const coursesToGrade = Object.keys(bulkMarks).filter(
      (id) => bulkMarks[id].internal && bulkMarks[id].semester,
    );
    ...
    try {
      const promises = coursesToGrade.map((courseId) =>
        axios.post(
          "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/marks/publish",
          {
      ...
      setRecordMessage(
        "✅ All Subject Marks successfully published to the Report Card!",
      );
      setBulkMarks({});
      ...
      if (err.response?.status === 403) {
        setRecordMessage(
          "❌ 403 Forbidden: Your session expired. Please log out and log back in.",
        );
      } else {
  ...
  const handleLogAttendance = (e) => {
    ...
    if (!recordStudentId || !recordCourseId) {
      setRecordMessage(
        "⚠️ Please select a Target Student and Course from the dropdowns first.",
      );
      return;
    }
    ...
    axios
      .post(
        "[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/attendance/log",
        {
          studentId: Number(recordStudentId),
  ...
  const handleViewStudentDetail = async () => {
    ...
    try {
      ...
      const [marksRes, attRes] = await Promise.all([
        axios
          .get(
            `[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/marks/student/${viewStudentId}`,
            { headers },
          )
          .catch(() => ({ data: [] })),
        axios
          .get(
            `[https://sms-backend-api-gqno.onrender.com](https://sms-backend-api-gqno.onrender.com)/api/erp/attendance/student/${viewStudentId}`,
            { headers },
          )
          .catch(() => ({ data: [] })),
      ]);
      ...
      setViewMarks(
        Array.isArray(marksRes.data) ? marksRes.data : marksRes.data.data || [],
      );
      setViewAttendance(
      ...
      alert(
        "Could not fetch academic records. Ensure the backend is running and endpoints exist.",
      );
    }
  };
  ...
  const downloadStudentsCSV = (e) => {
    e.preventDefault();
    let csvContent =
      "data:text/csv;charset=utf-8,Student ID,Full Name,Roll Number\n";
    students.forEach(
      (s) => (csvContent += `${s.id},${s.name},${s.rollNumber}\n`),
    );
  ...
  const downloadSingleStudentCSV = (e) => {
    e.preventDefault();
    if (!exportStudentId)
      return alert("Please select a student from the dropdown first!");
    ...
    link.setAttribute(
      "download",
      `${student.name.replace(/\s+/g, "_")}_Record.csv`,
    );
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  return (
    <div style={{ animation: "fadeIn 0.5s ease-out" }}>
      ...
      >
        ...
        >
          Manage the university's core infrastructure: subjects, staff,
      ...
      >
        ...
        {activeTab === "courses" && (
          <div style={{ animation: "appLaunch 0.4s ease-out" }}>
            <h3 style={{ marginTop: 0, fontSize: "22px", color: "#1d1d1f" }}>
              Add New Subject
            </h3>
            ...
            {courseMessage && (
              <div
                style={{
                  background: courseMessage.includes("✅")
                    ? "#dcfce7"
                    : "#fee2e2",
                  color:
                    courseMessage.includes("✅") || courseMessage.includes("🗑️")
                      ? "#166534"
                      : "#991b1b",
                  padding: "12px 20px",
                  borderRadius: "12px",
            ...
            >
              ...
              <select
                ...
                className="custom-input"
                style={{ flex: 1, minWidth: "120px", backgroundColor: "white" }}
                required
              >
        ...
        {activeTab === "faculty" && (
          <div style={{ animation: "appLaunch 0.4s ease-out" }}>
            ...
            {facMessage && (
              <div
                style={{
                  background: facMessage.includes("✅") ? "#dcfce7" : "#fee2e2",
                  color:
                    facMessage.includes("✅") || facMessage.includes("🗑️")
                      ? "#166534"
                      : "#991b1b",
                  padding: "12px 20px",
                  borderRadius: "12px",
              ...
              <tbody>
                {faculty.map((f) => (
                  <tr key={f.id}>
                    ...
                    <td>
                      <span className="dept-badge dept-it">{f.department}</span>
                    </td>
        ...
        {activeTab === "students" && (
          <div style={{ animation: "appLaunch 0.4s ease-out" }}>
            ...
            >
              ...
              >
                <div>
                  <h3 style={{ margin: 0, fontSize: "18px", color: "#1d1d1f" }}>
                  ...
                  >
                    Export all enrolled students or a specific student.
                  </p>
                </div>
                <button
              ...
              >
                ...
                >
                  <option value="">
                    -- Select a specific student to download --
                  </option>
                  {students.map((s) => (
            ...
            >
              ...
              <p style={{ margin: "0", fontSize: "13px", color: "#8e8e93" }}>
                Select a student to view their profile, report card, and
                attendance.
              </p>
                    ...
                    return (
                      <>
                        ...
                        {viewMarks.length === 0 ? (
                          ...
                          <div
                            style={{ overflowX: "auto", marginBottom: "25px" }}
                          >
                            <table
        ...
        {activeTab === "enrollment" && (
          <div style={{ animation: "appLaunch 0.4s ease-out" }}>
            <div style={{ display: "flex", flexWrap: "wrap", gap: "30px" }}>
              ...
              >
                <h3
                  style={{ marginTop: 0, fontSize: "22px", color: "#1d1d1f" }}
                >
                  1. Class Assignments
                </h3>
                <p
                ...
                >
                  Link a student to a course and professor.
                </p>

                {enrollMessage && (
                  <div
                    style={{
                      background: enrollMessage.includes("✅")
                        ? "#dcfce7"
                        : "#fee2e2",
                      color: enrollMessage.includes("✅")
                        ? "#166534"
                        : "#991b1b",
                      padding: "10px 15px",
                      borderRadius: "10px",
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) => setEnrollFacultyId(e.target.value)}
                    className="custom-select"
                    required
                  >
                    <option value="" disabled>
                  ...
                  <button
                    type="submit"
                    className="btn btn-add"
                    style={{ padding: "14px", marginTop: "5px" }}
                  >
                    🔗 Finalize
                  </button>
                </form>
              ...
              >
                <h3
                  style={{ marginTop: 0, fontSize: "22px", color: "#5856d6" }}
                >
                  2. Update Academic Data
                </h3>
                <p
                  style={{
                    color: "#8e8e93",
                ...
                >
                  Log attendance or bulk publish subject marks.
                </p>

                {recordMessage && (
                  <div
                    style={{
                      background: recordMessage.includes("✅")
                        ? "#e0e7ff"
                        : "#fee2e2",
                      color: recordMessage.includes("✅")
                        ? "#4338ca"
                        : "#991b1b",
                      padding: "10px 15px",
                      borderRadius: "10px",
                ...
                >
                  <select
                    ...
                    onChange={(e) => setRecordCourseId(e.target.value)}
                    className="custom-select"
                    style={{
                      flex: "1 1 100%",
                      borderColor: "#c7c7cc",
                  ...
                  <input
                    ...
                    className="custom-input"
                    style={{ flex: "1 1 120px", borderColor: "#c7c7cc" }}
                    required
                  />
                  <select
                    value={attStatus}
